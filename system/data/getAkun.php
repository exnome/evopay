<?php
	...
	if ($pos['posAkunStart']!=' ' and $pos['posAkunEnd']!=' ' and $Tipe=='BIAYA') {
		$akunBiaya = "and KodeGl between '".$pos['posAkunStart']."' and '".$pos['posAkunEnd']."'";
	} else {
		$akunBiaya = "";
	}

	if ($pos['posAkunHtgStart']!=' ' and $pos['posAkunHtgEnd']!=' ' and $Tipe=='HUTANG') {
		$akunHtg = "and KodeGl between '".$pos['posAkunHtgStart']."' and '".$pos['posAkunHtgEnd']."'";
	} else {
		$akunHtg = "";
	}
	...
	$sql = "select top $rp KodeGl,NamaGl from [$table]..glmst where KodeGl not in (
		select top $start KodeGl from [$table]..glmst where typerek = '02' $akunBiaya $akunHtg $search $sort
	) and typerek = '02' $akunBiaya $akunHtg $search $sort";
	// echo $sql;
	$result = mssql_query($sql);
	$total = mssql_num_rows(mssql_query("select KodeGl from [$table]..glmst where KodeGl=KodeGl and typerek = '02' $akunBiaya $akunHtg $search $sort"));
	$rows = array();

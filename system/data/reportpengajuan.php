	...
	foreach($rows as $dt) {
		//if ($data!=$dt['evo_id']) {
			...
			if ($dt['kodedealer']=='2010') {
				...
				$xml .= "<cell><![CDATA[".nbsp(note($dt['nobukti'],'DEPT. HEAD','FINANCE and ACCOUNTING','FINANCE'))."]]></cell>";
				$xml .= "<cell><![CDATA[".nbsp(note($dt['nobukti'],'DIV. HEAD','FINANCE and ACCOUNTING','all'))."]]></cell>";
			} else {
				$xml .= "<cell><![CDATA[".nbsp(note($dt['nobukti'],'SECTION HEAD','',''))."]]></cell>";

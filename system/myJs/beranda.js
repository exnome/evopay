jQuery(document).ready(function($) {
	...
	$('#thingstodo').click(function(event){
		...
		if (sesi_level=='ACCOUNTING') {
			onload = proval_accounting();
		} 
		if (sesi_level=='SECTION HEAD') {
			onload = proval_section();
		} 
		if (sesi_level=='DEPT. HEAD') {
			onload = proval_dept();
		}
		if (sesi_level=='DIV. HEAD') {
			onload = proval_div();
		}
		if (sesi_level=='DIREKSI') {
			onload = proval_direksi();
		} 
		if (sesi_level=='DIREKSI 2') {
			onload = proval_direksi2();
		}
		if (sesi_level=='FINANCE') {
			onload = proval_fin();			
		}
		if (sesi_level=='DEPT. HEAD' && user_div=='FINANCE and ACCOUNTING' && user_dept=='FINANCE') {
			onload = proval_releaser1();
		}
		if (sesi_level=='DIV. HEAD' && user_div=='FINANCE and ACCOUNTING' && user_dept=='all') {
			onload = proval_releaser2();			
		}
		/*
		if (srcLevel=='REQDEPTTERKAIT') {
			onload = proval_req(); 				
		}
		if (srcLevel=='WAITDEPTTERKAIT') {
			onload = proval_wait();
		}
		*/
	...
	$('#bucket_all').click(function(event){
		window.location.href='homebucket';
	});
});

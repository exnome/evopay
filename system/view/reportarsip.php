...
<div class="row">
	<form action="report-reportarsip-modify" id="Form" method="POST">
	    <span id="post"></span>
	    ...
	    <div class="col-sm-12">
	        ...
	        <script type="text/javascript">
	        	jQuery(document).ready(function($) {
                    ...
                    $('.flexme3').flexOptions({
						url : 'system/data/reportarsip.php',
						newp: 1,
					...
					function button(com) {
					    if (com == 'Export XLS') {
							...
							if (generallen>0) {
								...
								window.open('system/view/reportarsip_xls.php?id='+id+'&IdUser='+IdUser);
															//window.open('system/view/reportarsip_xls.php?nobukti='+nobukti+'&startDate='+startDate+'&endDate='+endDate+'&validasi='+validasi+'&noCsv='+noCsv+'&txtSearch='+txtSearch+'&IdUser='+IdUser+'&divisi='+divisi+'&department='+department+'&id='+id);
							} else {
								...
								window.open('system/view/reportarsip_xls.php?nobukti='+nobukti+'&startDate='+startDate+'&endDate='+endDate+'&validasi='+validasi+'&noCsv='+noCsv+'&txtSearch='+txtSearch+'&divisi='+divisi+'&department='+department+'&IdUser='+IdUser);
							}
							return true;
...
<?php
	function getDivisi($IdUser){
		...
		$sql = "select nama_div from sys_divisi where is_dealer='".$is_dealer."' and is_aktif=1";
		$rsl = mssql_query($sql);
	...
	function getDept($IdUser){
		...
		$sql = "select nama_dept from sys_department a inner join sys_divisi b on a.id_sys_div=b.id_sys_div where id_sys_dept=id_sys_dept and a.is_aktif=1 ";
		$rsl = mssql_query($sql);
		$opt = "<option value=\"\">- All -</option>";
		while ($dt = mssql_fetch_array($rsl)) {
